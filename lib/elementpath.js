...
function prepare_predicate(next, token) {
  ...
  if (token[1] === '@') {
    ...
    if (token[1]) {
      throw new Error('Invalid attribute predicate');
    }
    ...
    if (token[1] === ']') {
      ...
      if (value[0] === '"' || value[value.length - 1] === '\'') {
        ...
        throw new Error('Syntax error - invalid comparison target');
      }
    ...
    if (token[1] !== ']') {
      throw new Error('Syntax error - invalid attribute predicate');
    }
  }
  else if (!token[1]) {
    tag = token[0] || '';
    ...
    if (token[1] !== ']') {
      throw new Error('Syntax error - invalid node predicate');
    }
    ...
    throw new Error('Syntax error - invalid predicate');
  }
...
function findall(elem, path) {
  ...
  if (_cache.hasOwnProperty(path)) {
    ...
    if (path.charAt(0) === '/') {
      throw new Error('cannot use absolute path on element');
    }
